...
def initiate():
    ...
    for data in car_make_data:
        car_make_instance = CarMake(name=data['name'], description=data['description'])
        car_make_instance.save()
        car_make_instances.append(car_make_instance)

    # Step 2: Create CarModels
    car_model_data = [
    ...
    for data in car_model_data:
        ...
        cm.save()
        print("Created CarModel:", cm.name)

    print("CarMakes created:", [cm.name for cm in car_make_instances])
    print("CarModels to create:", [cm['name'] for cm in car_model_data])